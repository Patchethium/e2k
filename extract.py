...
import argparse
from e2k.constants import ascii_entries, kanas
...
def _extract_gptsovits():
    """
    extracts dictionary from userdict released in `gpt-sovits`
    a mecab userdict generated by LLM
    MIT license
    """
    path = "vendor/userdict.csv"
    f = open(path, "r")
    ...
    for row in tqdm(reader):
        ...
        if any([c not in ascii_entries for c in word]) or any([k not in kanas for k in katakana]):
            continue
        accent = int(accent.split("/")[0])
        katakana = list(katakana)
        katakana = katakana[:accent] + ["<acc>"] + katakana[accent:]
        res_dict[word].append(katakana)
    f.close()
    res_dict = filter_outliers(res_dict)
    return res_dict
...
if __name__ == "__main__":
    ...
    #             + "\n"
    #         )
    res_dict = _extract_gptsovits()
    f = open("vendor/katakana_dict.jsonl", "w")
    for k, v in res_dict.items():
        line = json.dumps({"word": k, "kata": v}, ensure_ascii=False)
        f.write(line + "\n")
    f.close()
